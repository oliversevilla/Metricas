...
function analizarMetricas($repo,$oa,$meta,$arrCatDC,$arrCatLom,$url,$tags,$xml,$dominio){
    //------------------------------
    //          DUBLIN CORE
    //------------------------------
    if(trim($url)!=''){//que url no sea vacio (para cuando se analiza varias url desde una BDD)
        if(isDcStd($url,$tags)) {

            //determinar si el oa (url) ya existe en tabla "meoa"
            $oaIdExist = $oa->exists($url);        
            if($oaIdExist>0) return -4;//"meoa" existente    
            else {    
                ...
                if($idRepo > 0){
                    ...
                    if($idOA > 0){                    
                        ...
                        for($i=0;$i<count($arrCatDC);$i++){

                            $etiquetaDC=$arrCatDC[$i]->ca_etiqueta;
                            ...
                            $completitudDC=$arrCatDC[$i]->ca_peso * $valCompDC;//al final en la formula sumar estos valores y actualizar en meoa
                            //---------------------------------------------------------------Evaluar formula Consistencia            
                            $instancia = new instancia(1,$arrCatDC[$i]->ca_id,0);//1 si es DUBLIN CORE
                            ...
                            $meta->insert($idRepo,$idOA,$arrCatDC[$i]->ca_id,-1,$contenidoDC,$etiquetaDC,$completitudDC,$consistenciaDC,$coherenciaDC);
                        }
                    ...
                    else return -2;//El OA no se pudo insertar (subir) en el servidor
                }
                else return -3;//El Repo no se pudo insertar (subir) en el servidor
            }
        }
        //------------------------------
        //              LOM
            ...
            if($oaIdExist>0) return -4;//"meoa" existente    
                ...
                if($idRepo > 0){        
                    ...
                    if($idOA > 0){                  
                        ...
                        for($i=0;$i<count($arrCatLom);$i++){

                            $etiquetaLOM=$arrCatLom[$i]->ca_etiqueta;//Ej: title
                            $subgrupoLOM=$arrCatLom[$i]->ca_subgrupo;//Ej: general
                            ...
                            $completitudLOM=$arrCatLom[$i]->ca_peso * $valCompLOM;//al final en la formula sumar estos valores y actualizar en meoa
                            //---------------------------------------------------------------Evaluar formula Consistencia            
                            $instancia = new instancia(2,$arrCatLom[$i]->ca_id,0);//2 si es LOM
                            $arrInstancia = $instancia->arregloInstancia;//arreglo de instancias con las que se comparara c/meta (formula)            
                            ...
                            $meta->insert($idRepo,$idOA,$arrCatLom[$i]->ca_id,-1,$contenidoLOM,$etiquetaLOM,$completitudLOM,$consistenciaLOM,$coherenciaLOM);
                        ...
                        //actualizo metrica Coherencia
                        $meta->updateCoherencia($idOA,round($coherenciaOchoa,2));
                        return $oa->getMaxId();//envio al FrontEnd un valor > 0
                    }
                    else return -2;//El OA no se pudo insertar (subir) en el servidor
                }
                else return -3;//El Repo no se pudo insertar (subir) en el servidor
